...
int main() {
    ...
    // Create socket
    sockfd = socket(AF_INET, SOCK_DGRAM, 0);/* TODO */
    

    // Set server address parameters
    server_addr.sin_family = AF_INET;/* TODO */       // IPv4
    server_addr.sin_addr.s_addr = htonl(INADDR_ANY);/* TODO */ // Any incoming interface
    server_addr.sin_port = htons(PORT);/* TODO */     // Server port

    // Bind the socket to the server address
    bind(sockfd, (struct sockaddr *) &server_addr, sizeof(server_addr)/* TODO */);

    printf("[INFO] server listening on port %d\n",PORT);

    while (1) {
        socklen_t len = sizeof(client_addr);
        ssize_t n = recvfrom(sockfd, buffer, BUFF_SIZE, 0, (struct sockaddr *) &client_addr, &len/* TODO */);
        buffer[n] = '\0';
        ...
        data->client_addr = client_addr;/* TODO */
        data->sockfd = sockfd;/* TODO */
        data->addr_len = len;/* TODO */
